...
class AppHueMejorada(Gtk.Window):
    ...
    def detectar_soporte_color(self, luz_info):
        ...
        modelos_blanco = [
            'LWB001', 'LWB004', 'LWB006', 'LWB007', 'LWB010',  # Blancas
            'LTW001', 'LTW002', 'LTW003', 'LTW004', 'LTW005',  # Blancas
            'LWF001', 'LWF002', 'LWF003',  # Blancas
            'LCA002', 'LCA003',  # Algunas pueden ser blancas
            'LWV001', 'LWV002',  # Lámparas de pared (generalmente blancas)
            'LTP001', 'LTP002',  # Tubos
            'LDD001', 'LDD002',  # Downlights
            'LST001',  # Tiras (blancas)
            'LLC001', 'LLC002', 'LLC003',  # Algunas tiras antiguas
        ]
        ...
        modelos_color = [
            ...
            'LCB001', 'LCB002',  # Algunas lámparas de mesa color
            'LCP001', 'LCP002',  # Lámparas de techo color
            'LCA001'  # Quitamos LWV001 de la lista de color
        ]
        
        # Obtener el modelo de la luz
        model_id = luz_info.get('modelid', '')
        
        # Primero verificar si es modelo blanco
        if model_id in modelos_blanco:
            return False
        
        # Luego verificar si es modelo color
        if model_id in modelos_color:
            return True
        
        # Verificar por capacidad en state
        state = luz_info.get('state', {})
        
        # Si tiene 'colormode', probablemente soporta color
        if 'colormode' in state:
            colormode = state.get('colormode')
            if colormode in ['hs', 'xy', 'ct']:
                # 'ct' es temperatura de color (blanco regulable), no RGB
                if colormode == 'ct':
                    return False
                return True
        
        # Verificar por tipo de producto
    ...
    def cargar_grupos_luces_y_escenas(self):
        ...
        try:
            ...
            # Obtener la API completa para tener información detallada de las luces
            api_completa = self.bridge.get_api()
            
            # Obtener información de cada luz (si soporta color)
            ...
            # Mostrar resumen de luces con color
    ...
    def crear_control_luz(self, luz):
        """Crea un widget de control para una luz individual en una sola fila con selector de color"""
        card = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
        card.get_style_context().add_class("card-luz")
        card.set_margin_bottom(2)
        
        # Nombre de la luz (sin icono extra)
        nombre = Gtk.Label()
    ...
    def on_color_clicked(self, widget, luz):
        ...
        try:
            # Crear diálogo de selección de color
            dialog = Gtk.ColorChooserDialog(title="Seleccionar color para la luz", parent=self)
            
            # Intentar obtener el color actual
            try:
                if hasattr(luz, 'xy') and luz.xy:
                    x, y = luz.xy
                    # Convertir xy a RGB para mostrar color actual
