...
class AppHueMejorada(Gtk.Window):
    def __init__(self):
        super().__init__(title="Philips Hue Controller v12.4 - Control de Luces y Sensores")
    ...
    def crear_interfaz(self):
        ...
        # Ícono SVG de batería dibujado con Cairo
        self._bat_color = (0.2, 0.8, 0.2)  # verde por defecto
        self._bat_drawing = Gtk.DrawingArea()
        self._bat_drawing.set_size_request(32, 18)
        self._bat_drawing.connect("draw", self._draw_battery_icon)

        lbl_bat_text = Gtk.Label(label="Estado de baterías")
        hbox_btn.pack_start(self._bat_drawing, False, False, 0)
        hbox_btn.pack_start(lbl_bat_text, False, False, 0)
        btn_baterias.add(hbox_btn)
        btn_baterias.connect("clicked", self.on_baterias_clicked)
        btn_baterias.set_margin_start(10)
        btn_baterias.set_margin_end(10)
        btn_baterias.set_margin_bottom(5)
        self.sidebar.pack_start(btn_baterias, False, False, 0)
    ...
    def _draw_battery_icon(self, widget, cr):
        """Dibuja un ícono de batería con Cairo del color de self._bat_color"""
        w = widget.get_allocated_width()
        h = widget.get_allocated_height()

        r, g, b = self._bat_color

        # Proporciones
        pole_w = w * 0.08    # tapa positiva
        pole_h = h * 0.35
        body_w = w - pole_w - 1
        body_h = h

        # Cuerpo de la batería (rectángulo redondeado)
        radius = 2.5
        x, y = 0, 0
        bw, bh = body_w, body_h
        ...
        cr.arc(x + bw - radius, y + radius,      radius, 3*3.14159/2, 0)
        cr.arc(x + bw - radius, y + bh - radius, radius, 0, 3.14159/2)
        cr.arc(x + radius,      y + bh - radius, radius, 3.14159/2, 3.14159)
        cr.close_path()
        ...
        # Relleno interior (más oscuro)
        cr.set_source_rgba(r * 0.25, g * 0.25, b * 0.25, 1.0)
        cr.fill()

        # Barra de carga interior
        padding = 3
        nivel = self._bat_nivel if hasattr(self, '_bat_nivel') else 100
        fill_w = max(0, (body_w - padding * 2) * (nivel / 100))
    ...
    def _crear_tarjeta_bateria(self, datos):
        ...
        icono.connect("draw", self._draw_battery_card_icon, color, nivel)
    ...
    def _draw_battery_card_icon(self, widget, cr, color, nivel):
        """Dibuja ícono de batería en las tarjetas de la ventana de baterías"""
        w = widget.get_allocated_width()
        h = widget.get_allocated_height()
        r, g, b = color
    ...
    def crear_tarjeta_sensor(self, sensor_data):
        ...
        last_up = Gtk.Label()
        last_up.set_markup(f"<span size='x-small' foreground='#cccccc'>🕐 {sensor_data['last_updated'].strftime('%H:%M:%S')}</span>")
